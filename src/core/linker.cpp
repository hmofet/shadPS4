    ...
    main_thread.Run([this, module, &args, has_libcinternal](std::stop_token) {
        ...
        if (has_libcinternal) {
            ...
            if (malloc_init && sceLibcInternalMemoryMutexEnable) {
                // Call _malloc_init
                s32 ret = Core::GuestCall("_malloc_init", malloc_init);
                ASSERT_MSG(ret == 0, "malloc_init failed");
                Core::GuestCall("sceLibcInternalMemoryMutexEnable", sceLibcInternalMemoryMutexEnable);
            }
        }
...
void* Linker::TlsGetAddr(u64 module_index, u64 offset) {
    ...
    if (!addr) {
        ...
        if (heap_api && heap_api->heap_malloc) {
            dest = reinterpret_cast<u8*>(Core::GuestCall("heap_malloc", heap_api->heap_malloc, module->tls.image_size));
        } else {
...
void* Linker::AllocateTlsForThread(bool is_primary) {
    ...
    if (is_primary) {
        ...
        if (heap_api && heap_api->heap_malloc) {
            addr_out = Core::GuestCall("heap_malloc", heap_api->heap_malloc, total_tls_size);
        } else {
...
void Linker::FreeTlsForNonPrimaryThread(void* pointer) {
    if (heap_api && heap_api->heap_free) {
        Core::GuestCall("heap_free", heap_api->heap_free, pointer);
    } else {
        std::free(pointer);
    }
}
